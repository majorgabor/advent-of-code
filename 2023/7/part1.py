...
def get_type(cards) -> Type:
    multiple_occurrences = [i for i in Counter(cards).values() if i > 1]
    if len(multiple_occurrences) == 0:
        if card_order.find("".join(cards)) > -1:
            return Type.HIGH_CARD
        return Type.NONE
    if 5 in multiple_occurrences:
        return Type.FIVE_OF_KIND
    if 4 in multiple_occurrences:
        return Type.FOUR_OF_KIND
    if all(x in multiple_occurrences for x in [3, 2]):
        return Type.FULL_HOUSE
    if 3 in multiple_occurrences:
        return Type.THREE_OF_KIND
    if 2 in Counter(multiple_occurrences).values():
        return Type.TWO_PAIR
    if 2 in multiple_occurrences:
        return Type.PAIR
