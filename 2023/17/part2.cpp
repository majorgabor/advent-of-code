...
{
    const Step source_step = {source, {Direction::DOWN, 0}};
    ...
    while (!priority_queue.empty())
    {
        ...
        for (const auto &move : moves)
        {
            ...
            {
                if (current_step.second.first != (Direction)(((int)next_step.second.first + 2) % 4) && next_step.second.second <= 10)
                {
                    if ((next_step.second.second == 1 && 4 <= current_step.second.second) || 4 < next_step.second.second || current_step.second.first == next_step.second.first)
                    {
    ...
    for (const auto direction : {Direction::DOWN, Direction::RIGHT})
    {
        for (int i = 4; i <= 10; i++)
        {
            if (cumulative_lava_loss.find({{mat.size() - 1, mat[0].size() - 1}, {direction, i}}) != cumulative_lava_loss.end())
            {
